...
//                Cov(X,Y)=Cov(X,Y)*cov_sd(i,X)*cov_sd(i,Y);
      ...
      double meta_logP;
      uvec Indice_1 = conv_to< uvec >::from(index); 
      if(index.size()>0 && Cov.n_rows-index.size()>=minimum_tissue)
       {
         Cov.shed_cols(Indice_1); 
         Cov.shed_rows(Indice_1);     
...
//      cout<<"TEST"<<endl;
//      cout<<gsl_cdf_ugaussian_Pinv (0.5)<<endl;
//      cout<<"OVER"<<endl;
//      cout<<"Han is: "<<Han<<endl;
...
//         cout <<"estimated p_Han_adjust is: "<<p_Han_adjust<<endl; 
       }
...
//   cout<<"Infor2 is "<<z_score_[0][0].size()<<" "<<beta_[0][0].size()<<" "<<sd_[0][0].size()<<" "<<variant_[0].size()<<endl; 
   ...
   double P=ConstructCovRandom(z_score_, beta_, sd_ ,index , Res_fixed, Res_random, meta_mode, output1, variant_, pos, pos1, Res, MAF, VARIANCE, data_index, test1, test2,test3, ALLELE, cor_file, write_cor,  minimum_tissue,  Han);
//   cout<<"P is: "<<P<<endl;
   if(P<0)
    {
...
double MetaAnalysis(vector<vector<vector<double> > > &z_score_, vector<vector<vector<double> > >&beta_, vector<vector<vector<double> > >&sd_,int index, mat &Res_fixed, mat &Res_random, string meta_mode, string output, vector<vector<string> > &variant_, map<string, bool> &pos, int colocalization, map<string, string> &GWAS_file, map<string, double > &MAF, vector<double > &VARIANCE, map<int,bool> &data_index,string gene,vector<double> &test1_1, vector<double> &test2_1, vector<double> &test3_1, map<string, string> &ALLELE, string cor_file, bool write_cor, int minimum_tissue, bool Han)
 {
//   cout<<"Come into MetaAnalysis function to perform final meta-analysis"<<endl;
   string output1= output+"_statistical_signal";
   map <string, bool> pos1;
...
//   cout<<"Perform real meta-analysis"<<endl;
   ...
   double P=ConstructCovRandom(z_score_, beta_, sd_ ,index , Res_fixed, Res_random, meta_mode, output1, variant_, pos, pos1, Res, MAF, VARIANCE, data_index,test1_1,test2_1, test3_1, ALLELE, cor_file, write_cor, minimum_tissue, Han);
      ...
      for(int I=0, indx=0;I<file_.size();)
       {
         ...
         read_allele(allele_file_[I],allele);
          ...
          {
            cout<<"Read summary result from file"<<file_[I]<<endl;
         ...
         if(nested>0 && nested_1 >1 && file_.size()>nested)
          {
            ...
            string test_file=""; 
            MetaAnalysis(z_score_nested,  beta_nested, sd_nested, x, Res_fixed, Res_random, "fixed", Output1, variant_nested, pos, 0,GWAS_file, MAF, VARIANCE, data_index1, gene, test1_1, test2_1, test3_1, ALLELE, test_file,false, minimum_tissue, Han);
            ...
            bool INDEX=true;
            P=MetaAnalysis(z_score_nested2,  beta_nested2, sd_nested2, x, Res_fixed_2, Res_random_2, "random", Output2, variant_nested2, pos, 0,GWAS_file, MAF, VARIANCE2, data_index2, gene, INDEX, finemap, ALLELE,p_cutoff,cor_file,false, minimum_tissue, Han);
          }
//          cout<<"Here3_2"<<endl; 
...
//          cout<<z_score_[0][0].size()<<endl;
          P1=MetaAnalysis(z_score_,  beta_, sd_, x, Res_fixed, Res_random, meta_mode, Output, variant_, pos, colocalization,GWAS_file, MAF, VARIANCE, data_index, gene, INDEX, finemap, ALLELE,p_cutoff,cor_file, write_cor, minimum_tissue,Han);
          write_cor=false;
//          cout<<"Here3_3"<<endl;
          if(cis_summary)
           {
...
//                string PEAK=variant_[indx][int(Res_fixed_2(0,0))];
  ...
  //                 cout<<"Fuck, there is no genotype infor of causal variant in this data set"<<i<<endl;
               ...
               int COL=VAR[PEAK];
    //           cout<<"col for causal variant is: "<<COL<<", and peak variant is: "<<PEAK<<endl;
               if(nested>0 && nested_1>1)
                {
...
//FOR TEST
//    P=0.01;
//         cout<<"Only primary or not"<<endl;
//         cout<<"eQTL_number is: "<<eQTL_number<<endl;
         ...
         if(covariate!="NA")
          {
            ...
            cPar.file_cvt=str;         
//Changed by Biao Zeng, 09/10/2021, 6:11 pm 
      //      cPar.maf_level=-1;
          }
