       ...
       while(Bim && getline(Bim, A))
        {
          ...
          string var;
          ss>>chr;
                             if (regex_match (chr,regex("^(chr)(.*)") ) || regex_match (chr,regex("^(Chr)(.*)") ) )
                    {
                      chr.erase(chr.begin(),chr.begin()+3);
...
//           int b=0;
           ...
           while(FHIN1 &&getline(FHIN1,a))
            {
               stringstream ss(a);
                  ...
                  int Start;
                  ss>>c;
                  if (regex_match (c,regex("^(chr)(.*)") ) || regex_match (c,regex("^(Chr)(.*)") ) )
                   {
           ...
           FHIN1.clear();
           FHIN1.close();
//           cout<<"Read annotation is over"<<endl;
         }
...
//           cout<<"Read genotype is over"<<endl;
