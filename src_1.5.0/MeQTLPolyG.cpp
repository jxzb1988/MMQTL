...
void output_version(){
    ...
    cout<<"version 1.5.0 "<<endl;
    cout<<"Copyright 2020-present, Icahn School of Medicine at Mount Sinai"<<endl;
    cout<<"Under GNU General Public v3 License"<<endl;
    cout<<"Please report bugs to Biao Zeng <biao.zeng@mssm.edu>"<<endl;
}
...
//                        str.clear();
  //                      str.assign(argv[i]);
               ...
               else if(strcmp(argv[i], "-out")==0 || strcmp(argv[i], "--out")==0 ||strcmp(argv[i], "-output")==0  || strcmp(argv[i], "--output")==0 || strcmp(argv[i], "-o")==0) {
                        if(argv[i+1] == NULL || argv[i+1][0] == '-') {continue;}
                        ++i;
                        str.clear();
                        str.assign(argv[i]);
...
//                        if(argv[i+1] == NULL || argv[i+1][0] == '-') {continue;}
  ...
  //                      if(argv[i+1] == NULL || argv[i+1][0] == '-') {continue;}
               ...
               else if(strcmp(argv[i], "-cov")==0 || strcmp(argv[i], "--cov")==0) {
                 COV =true;
                }
                else if(strcmp(argv[i], "-wald")==0 || strcmp(argv[i], "--wald")==0) {
                   wald=true;
                }
                else if(strcmp(argv[i], "-score")==0 || strcmp(argv[i], "--score")==0) {
                   wald=false;
                }
...
//                        str.clear();
  //                      str.assign(argv[i]);
               ...
               else if(strcmp(argv[i], "-z_cutoff")==0 || strcmp(argv[i], "--z_cutoff")==0 || strcmp(argv[i], "-V")==0) {
                        if(argv[i+1] == NULL || argv[i+1][0] == '-') {continue;}
                        ++i;
                        z_cutoff =atoi(argv[i]);
                }
                ...
                else if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "--help")==0  || strcmp(argv[i], "--h")==0 || strcmp(argv[i], "-h")==0) {
                 
                                cout << "Options: " << endl;
                                cout << "-h, --help                     show this help message and exit " << endl;
//                                cout << "-I,                     Perform colocalization analysis " << endl;
//                                cout << "-b,                      " << endl;
                                cout << "-o STRING,      specify the output file" << endl;
                                cout << "-z_cutoff DOUBLE, --z_cutoff DOUBLE, specify absolute z_score cutoff used to estimate covariance, the default is 2" << endl;
                                cout << "-p_cutoff DOUBLE, --p_cutoff DOUBLE, specify p-value cutoff to detect independent QTL when conducting conditional analysis, the default is 0.000001" << endl;
  //                              cout << "-l LDFILE, --ld_file=LDFILE    the ld input file" << endl;
                                cout << "-p FILE, --y_file FILE, set the file which contains paths to phenotype files" << endl;
                                cout << "-covariate FILE, --covariate FILE, set the file which contains paths to covariate files" << endl;
...
//      //                          cout << "-g GAMMA, --gamma              set $gamma$ the prior of a SNP being causal (default 0.01)" << endl;
  //                              cout << "-c causal                      set the maximum number of causal SNPs" << endl;
                                cout << "-C FILE, --covariate FILE,      set the covariate matrix "<<endl;
                                cout << "-a FILE, set gene annotation file"<<endl;
                                cout << "-cov, --cov,  use cov to estimate covariance due to sample overlap. The default is cor"<<endl;
                                cout << "-T STRING, --target STRING, set the feature to explore "<<endl;
//                                cout << "-M 0/1,       run in meta manner or not "<<endl;
                                cout << "-grm_file FILE, --grm_file FILE, set the file which contains paths to plink-format genotype files "<<endl;
                                cout << "-A STRING, set Meta analysis pattern, either fixed or random "<<endl;
//                                cout << "-f 1                           to out the probaility of different number of causal SNP" << endl;
  //                              cout << "-w Weight file, --weight       set the biological annotation to use" << endl;
                                cout << "-V INTEGER, set the cis windown around features,  the default is 1Mb (1Mb upstream and 1Mb downstream to features)"  << endl;
                                cout << "-B INTEGER, set the window around features, whthin which, variants are used to estimate covariance, the default is 3Mb (3Mb upstream and 3MB downstream to features)" << endl;
                                cout << "-eQTL_number INTEGER, --eQTL_number INTEGER, set maximum number of eQTL to report" << endl;
                                cout << "-primary_only, --primary_only, focus only on primary QTL"<<endl;
                                cout << "-wald, --wald, use Wald test to assess significance in mixed linear model, the default is Wald test        " << endl;
                                cout << "-score, --score, use Score test to assess significance in mixed linear model, the default is Wald test        " << endl;
                                cout << "-Han, apply Han & Eskin method to adjust results of random-effect model"<<endl;
//                                cout << "-S GWAS summary statistic results," << endl;
             ...
             if(!test_only)
              {
               ...
               MM_conditional_analysis(y,gene, totalCausalSNP,rho, histFlag, gamma,weight, nthread,covariate_file_, grm_file_,outputFileName, Geno_file_, meta_mode, allele_file_, pos, colocal, gwas_file_, MAF, data_index, nested, finemap,p_cutoff, cis_summary,cor_file, causal, eQTL_number, Han,wald, z_cutoff, COV);
...
//Prepare_input(string input_1_causal, string geno_file_1_causal, string gene, string anno, string input_1, vector<string> &allele_file_,vector<string> &y,map<string, bool> &pos, map<int, bool> &data_index,int colocal, bool finemap, int cis_length, int cov_length, vector<string> covariate_1, int LD_index);
               ...
               MM_conditional_analysis(y_1,gene, totalCausalSNP,rho, histFlag, gamma,weight, nthread,covariate_1, grm_file_1,outputFileName, Geno_file_, meta_mode, allele_file_1, pos, colocal, gwas_file_, MAF, data_index, nested, finemap,p_cutoff, cis_summary,cor_file_1, causal_1, eQTL_number, Han, wald, z_cutoff, COV);
...
//              {
              if(!cis_summary)
               {
  //               cout<<"Come to prepare input file"<<endl;
                 int state =Prepare_input(input, geno_file, gene, anno, input_, allele_file_,y,pos,data_index,colocal, finemap, cis_length, cov_length,covariate_, LD_index);
                 if(state!=0)
                  {
                    cout<<"Please have a check on input genotype or phenotype"<<endl;
                    return -1;
                  }
    //             cout<<"Preparing is over BZ"<<endl;
               } else
               {
...
//              }
      //        cout<<"Prepare input is over"<<endl;
              if(Grm_file=="")
               ...
               {
                 vector<string> grm_file_tem1;
                 vector<string> grm_file_tem;

                 ReadInputFile(Grm_file, grm_file_tem1);
                 if(grm_file_tem1.size()!=data_index.size())
                  {
                    cout<<"The number of dataset with genotype and grm should be matched. Please have a check. EXIT"<<endl;
                    exit(EXIT_FAILURE);
                  }
                 for(int i=0;i<data_index.size();i++)
                  {
                    if(data_index[i])
                     {
                       cout<<grm_file_tem1[i]<<endl;
                       grm_file_tem.push_back(grm_file_tem1[i]);
                     } else
                     {
                       grm_file_tem.push_back(string("NA"));
                     }
                  }
...
//                 covariate_.push_back(string("NA"));
   ...
   //         cout<<"Output grm file infor:"<<endl;
...
//               cout<<"Preparing input file is over"<<endl;
               MM_conditional_analysis(y_1,gene, totalCausalSNP,rho, histFlag, gamma,weight, nthread,covariate_2, grm_file_1,outputFileName1, Geno_file_, meta_mode, allele_file_1, pos, -1, gwas_file_, MAF, data_index2, nested, false,p_cutoff, cis_summary,cor_file_1, causal_1, eQTL_number, Han, wald, z_cutoff, COV);
